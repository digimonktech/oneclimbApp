...
const AllCoaches = ({ modalRef, coaches }) => {
  const db = getFirestore(firebase.initializeApp(firebaseConfig));
  const [searchText, setSearchText] = useState();
  const [searchList, setSearchList] = useState(coaches);
  const searchInputRef = useRef();

  useEffect(() => {
    setSearchList(coaches);
  }, [coaches]);

  const searchCoaches = async (value) => {
    let searchArray = [];
    ...
    <Portal>
      ...
      >
        <View style={styles.container}>
          <Text style={styles.title}>All Coaches</Text>
          <FlatList
